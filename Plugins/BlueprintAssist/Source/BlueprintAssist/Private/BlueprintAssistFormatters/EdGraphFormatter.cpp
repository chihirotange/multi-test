...
void FEdGraphFormatter::ExpandPendingNodes(bool bUseParameter, const TArray<TSharedPtr<FFormatXInfo>>& NodesToExpand)
{
...
void FEdGraphFormatter::FormatX(const bool bUseParameter)
{
	...
	// Nodes which do not have execution to the root node need to be expanded and should be formatted as input direction
	TSet<UEdGraphNode*> OutputNodeTree = FBAUtils::GetNodeTree(RootNode, EGPD_Output);
	TSet<UEdGraphNode*> NodePoolSet = TSet<UEdGraphNode*>(NodePool);
	TSet<UEdGraphNode*> InputFacingNodes = NodePoolSet.Difference(OutputNodeTree);
	...
	TQueue<TSharedPtr<FFormatXInfo>> OutputStack;
	TQueue<TSharedPtr<FFormatXInfo>> InputStack;
	OutputStack.Enqueue(RootInfo);
	FormatXInfoMap.Add(RootNode, RootInfo);

	Path.Reset();

	DecideXParents(bUseParameter, OutputStack, InputStack, DirtyNodes, NodesToExpand, InputFacingNodes);

	if (UBASettings::Get().FormattingStyle == EBANodeFormattingStyle::Expanded)
	{
		for (TSharedPtr<FFormatXInfo> FormatXInfo : NodesToExpand)
		{
			OutputStack.Enqueue(FormatXInfo);
			DirtyNodes.Add(FormatXInfo);
		}

		ExpandPendingNodes(bUseParameter, NodesToExpand);
...
void FEdGraphFormatter::DecideXParents(
	...
	TSet<TSharedPtr<FFormatXInfo>>& DirtyNodes,
	TArray<TSharedPtr<FFormatXInfo>>& NodesToExpand,
	const TSet<UEdGraphNode*>& InputFacingNodes)
{
	while (!OutputStack.IsEmpty() || !InputStack.IsEmpty())
	{
		...
		{
			OutputStack.Dequeue(CurrentInfo);
		}
		else
		{
			InputStack.Dequeue(CurrentInfo);
		}

		UEdGraphNode* CurrentNode = CurrentInfo->GetNode();

		// UE_LOG(LogBlueprintAssist, Warning, TEXT("Processing %s"), *CurrentInfo->Link.ToString());
			...
			if (bHasNoParent || !bHasCycle)
			{
				if (OldInfo->Parent.IsValid())
				{
					...
					if (!bTakeNewParent)
					{
						...
						// UE_LOG(LogBlueprintAssist, Warning, TEXT("\t Comparing parents Old: %s (%d) New: %s (%d) (%d)"),
						...
						bTakeNewParent = bPositionIsBetter;
					}
		...
		bool bShouldIterate = bHasChanged || CurrentNode == RootNode;

		if (DirtyNodes.Contains(CurrentInfo))
		{
			DirtyNodes.Remove(CurrentInfo);
			bShouldIterate = true;
		}

		if (!bShouldIterate)
		{
			// UE_LOG(LogTemp, Warning, TEXT("\tHas not changed, skipping!"));
			continue;
		}
		...
		for (int i = LinkedPins.Num() - 1; i >= 0; --i)
		{
			...
			for (UEdGraphPin* LinkedPin : ParentPin->LinkedTo)
			{
				...
				// UE_LOG(LogBlueprintAssist, Warning, TEXT("\t\t\tIterating pin link %s"), *PinLink.ToStringConst());

				TSharedPtr<FFormatXInfo> LinkedInfo = MakeShared<FFormatXInfo>(PinLink, CurrentInfo);

				if (ParentPin->Direction == EGPD_Output)
				{
					OutputStack.Enqueue(LinkedInfo);
					// UE_LOG(LogBlueprintAssist, Warning, TEXT("\t\t\t\t\tAdded to output stack"));
				}
				else
				{
					if (InputFacingNodes.Contains(LinkedInfo->GetNode()))
					{
						if (!InputFacingNodes.Contains(CurrentInfo->GetNode()))
						{
							NodesToExpand.Add(CurrentInfo);
						}

						InputStack.Enqueue(LinkedInfo);
